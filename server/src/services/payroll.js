...
//   - Sick leave:          sickDays × avgDailyHours × wage
//   - UIF:                 1% of gross
//
...
export function calcPayslip({
  ...
  for (const d of attendance || []) {
    ...
    if (d.type === 'normal') {
      ...
      sickHours += hours;
      sickDays++;
    } else if (d.type === 'annual' || d.type === 'unpaid') {
      leaveHours += hours;
    }
  }
  ...
  const sickPay = round2(sickDays * avgDaily * w);
...
export function ytdForEmployee(employee, payslipsBeforeAndIncluding) {
  const acc = (payslipsBeforeAndIncluding || []).reduce(
    ...
    { gross: 0, uif: 0, normal: 0, overtime: 0, holiday: 0, publicHoliday: 0, sick: 0, commission: 0, bonus: 0 },
  );
